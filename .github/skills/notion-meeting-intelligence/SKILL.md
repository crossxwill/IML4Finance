...
description: Prepare meeting materials with Notion context and external research; use when gathering context, drafting agendas/pre-reads, and tailoring materials to attendees.
metadata:
...
Prep meetings by pulling Notion context, tailoring agendas/pre-reads, and enriching with external research.
...
5) Enrich with external research (industry insights, benchmarks, risks) and update the page with `Notion:notion-update-page` as plans change.

## Workflow
### 0) If any MCP call fails because Notion MCP is not connected, pause and set it up:
1. Add the Notion MCP:
   - `<mcp-client> mcp add notion --url https://mcp.notion.com/mcp`
2. Enable remote MCP client:
   - Set `[features].rmcp_client = true` in `config.toml` **or** use your MCP client's equivalent flag (for example: `<mcp-client> --enable rmcp_client`)
3. Log in with OAuth:
   - `<mcp-client> mcp login notion`

After successful login, the user may need to restart their MCP client. You should finish your answer and tell them so when they try again they can continue with Step 1.
...
- Add concise external research where helpful: market/industry facts, benchmarks, risks, best practices.
