...
# Cap CPU usage during rendering to reduce memory pressure.
num_jobs = CPU_LIMIT
...
```{python}
model_folder = '../Artifacts/models/Lab01_ag_models_NoTransform'
...
```{python}
# Get the actual best model from the predictor
best_model_name = ag_model.named_steps['autogluon'].predictor.model_best
...
cal_disp = CalibrationDisplay.from_estimator(
        ...
        y = test_data[label],
        n_bins = 20,
        name = f'Uncalib_{best_model_name}',
        color = 'orange'
    )
...
filter_model_folder = '../Artifacts/models/Lab01_ag_models_FilterMethod'
...
```{python}
# remove ag folder if exists
embedded_model_folder = '../Artifacts/models/Lab01_ag_models_EmbeddedMethod'
...
ag_model_embedded_method = Pipeline([
    ...
    ('feature_selection', SelectFromModel(
        ExtraTreesClassifier(random_state=2025, criterion='entropy', max_depth=3, n_jobs=num_jobs),
        threshold=0.01
    )),
    ('autogluon', AutoGluonSklearnWrapper(label=label,
                                    predictor_args={'problem_type': 'binary',
...
```{python}
# remove ag folder if exists
wrapper_model_folder = '../Artifacts/models/Lab01_ag_models_WrapperMethod'
