...
-   **ROC-AUC** is a common binary classification metric, but it can be misleading when classes are highly imbalanced:
    ...
    -   FPR is calculated as $\frac{\text{False Positives}}{\text{All Actual Negatives}}$. When the negative class (legitimate transactions) is very large, even a substantial number of false positives may result in a low FPR, making the ROC curve appear optimistic.
    -   While a model predicting only "not fraud" would have ROC-AUC of 0.5 (random performance), the more important issue is that ROC-AUC can appear reasonably high even for models that perform poorly on the minority class, because high accuracy on the majority class can mask deficiencies in minority class detection.
-   **Precision-Recall (PR) curves** are more informative for imbalanced data:
    -   The PR curve plots Precision ($\frac{\text{True Positives}}{\text{True Positives} + \text{False Positives}}$) against Recall ($\frac{\text{True Positives}}{\text{True Positives} + \text{False Negatives}}$).
