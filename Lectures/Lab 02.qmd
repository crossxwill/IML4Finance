...
# 8. Evaluation & Scoring the Test Set
...
# Remove non-numeric chars from dti in df_rejected_test before processing
df_rejected_test['Debt-To-Income Ratio'] = df_rejected_test['Debt-To-Income Ratio'].astype(str).str.replace(r'[^0-9\.\-]', '', regex=True)
df_rejected_test['Debt-To-Income Ratio'] = pd.to_numeric(df_rejected_test['Debt-To-Income Ratio'], errors='coerce')
...
# 3. Calculate Weights for Rejected Test Data
print("Calculating weights for rejected test data using RI model...")

X_rej_test_ri = df_rejected_test_common[ri_features].copy()
weights_bad = ri_model.predict_proba(X_rej_test_ri)[:, 1]
weights_good = 1.0 - weights_bad

# 4. Create two copies of the rejected test data set and assign weights

# Copy 1: Assumed Bad (default_flag = 1)
df_rejected_test_bad = df_rejected_test_common[modeling_features].copy()
df_rejected_test_bad['sample_weight'] = weights_bad
df_rejected_test_bad['default_flag'] = 1
df_rejected_test_bad['original_source'] = 'Rejected' # Track original source

# Copy 2: Assumed Good (default_flag = 0)
df_rejected_test_good = df_rejected_test_common[modeling_features].copy()
df_rejected_test_good['sample_weight'] = weights_good
df_rejected_test_good['default_flag'] = 0
df_rejected_test_good['original_source'] = 'Rejected' # Track original source

# 5. Prepare Accepted Test Data with Weight and Source
df_accepted_test_weighted = df_accepted_test_common[modeling_features + [target_col]].copy()
df_accepted_test_weighted['sample_weight'] = 1.0
df_accepted_test_weighted['original_source'] = 'Accepted' # Track original source

# 6. Concatenate to create the full TTD test set for evaluation
cols_for_ttd_test = modeling_features + ['default_flag', 'sample_weight', 'original_source']
df_ttd_test_full = pd.concat(
    [df_accepted_test_weighted[cols_for_ttd_test],
     df_rejected_test_bad[cols_for_ttd_test],
     df_rejected_test_good[cols_for_ttd_test]],
    ignore_index=True
)

print(f"TTD test set created. Shape: {df_ttd_test_full.shape}")

# 7. Summarize the TTD Test Set by Source
if 'df_ttd_test_full' in locals() and df_ttd_test_full is not None:
    print("\n--- Summary of TTD Test Set by Original Source ---")
    # Step 1: Calculate standard aggregations
    summary_stats_test = df_ttd_test_full.groupby('original_source').agg(
        row_count=('default_flag', 'size'),
        sum_weights=('sample_weight', 'sum'),
        unweighted_default_rate=('default_flag', 'mean')
    ).reset_index()
    ...
    # Step 2: Calculate weighted default rate separately using apply
    weighted_rates_test = df_ttd_test_full.groupby('original_source').apply(
        lambda x: np.average(x['default_flag'], weights=x['sample_weight']) if x['sample_weight'].sum() > 0 else np.nan
    ).reset_index(name='weighted_default_rate')

    # Step 3: Merge the results
    summary_default_rates_test = pd.merge(summary_stats_test, weighted_rates_test, on='original_source')

    # Display the summary
    print("Summary of Default Rates by Original Source (Test Set):")
    display(summary_default_rates_test[['original_source', 'row_count', 'unweighted_default_rate', 'sum_weights', 'weighted_default_rate']])
else:
    print("Skipping TTD test set summary: df_ttd_test_full not available.")


# 8. Evaluate the Constrained Model on the TTD Test Set
if 'ag_model_constrained_wrapped' in locals() and ag_model_constrained_wrapped is not None and \
   'df_ttd_test_full' in locals() and df_ttd_test_full is not None:
    print("\n--- Evaluating Constrained Model on TTD Test Set ---")
    
    # Prepare features for prediction
    X_test_ttd = df_ttd_test_full[modeling_features]
...
#| tbl-number: true
#| code-summary: "Generate a KS table based on scores for the TTD test data."

if df_test_results is not None and 'score' in df_test_results.columns and 'sample_weight' in df_test_results.columns:
    ...
    print("Skipping KS table generation: Required columns not available in df_test_results.")
...
```{python}
#| label: plot-cumulative-rates
#| #| code-summary: "Plot cumulative bad rates and good rates to visualize the KS statistic."
if ks_results_table is not None and not ks_results_table.empty:
    ...
    avg_scores = ks_results_table['avg_score']
    ...
    plt.xlabel('Average Score (per Bin)')
    ...
    print("Skipping cumulative rates plot: KS results table is not available.")
```

**Interpretation:** A KS > 30 is often acceptable, > 40 good, > 50 excellent.
...
Calculate potential thresholds using different optimization criteria on the *TTD test data*.
...
#| code-summary: "Generate counterfactual explanations for rejected applicants using DiCE."

# Check if dice_ml is imported and necessary data is available
if dice_ml is not None and 'df_test_results' in locals() and df_test_results is not None and \
    'ag_model_constrained_wrapped' in locals() and ag_model_constrained_wrapped is not None and \
    'selected_threshold_prob' in locals() and 'modeling_features' in locals():

     print("--- Generating Counterfactual Explanations ---")
     dice_setup_ok = False
     try:
          # 1. Prepare data for DiCE
          # DiCE needs features and the outcome ('decision')
          dice_data = df_test_results[modeling_features + ['decision']].copy()
          
          # Define continuous features for DiCE
          continuous_features_dice = modeling_features # All our modeling features are numeric

          # 2. Create DiCE Data object
          d = dice_ml.Data(dataframe=dice_data, 
                                 continuous_features=continuous_features_dice, 
                                 outcome_name='decision')

          # 3. Create DiCE Model object using the wrapped AutoGluon model
          # We need a predict function that outputs 'Approve'/'Reject' based on the threshold
          def dice_predict_func(X):
                # Ensure input is DataFrame with correct feature names
                if isinstance(X, np.ndarray):
                     X_df = pd.DataFrame(X, columns=modeling_features)
                else:
                     # Ensure columns are in the correct order if it's already a DataFrame
                     X_df = X[modeling_features].copy() # Use copy to avoid warnings
                
                # Predict probabilities using the wrapped model
                probabilities = ag_model_constrained_wrapped.predict_proba(X_df)[:, 1]
                # Convert probabilities to decisions based on the selected threshold
                decisions = np.where(probabilities < selected_threshold_prob, 'Approve', 'Reject')
                 return decisions

          # Wrap the prediction function for DiCE
          # Provide a dummy model_path="" to avoid TypeError in dice-ml when func is used
          m = dice_ml.Model(model_path="", 
                                  backend='sklearn', # Use sklearn backend as wrapper is compatible
                                  func=dice_predict_func) # Provide the custom prediction function

          # 4. Initialize DiCE explainer
          # Method 'random' is simple; 'kdtree' or 'genetic' can be more sophisticated
          dice_explainer = dice_ml.Dice(d, m, method="random") 
          print("DiCE explainer initialized.")

          # 5. Select some rejected applicants to explain
          rejected_applicants = df_test_results[df_test_results['decision'] == 'Reject']
          
          if not rejected_applicants.empty:
                num_cf_examples = min(3, len(rejected_applicants)) # Explain up to 3
                applicants_to_explain = rejected_applicants.sample(num_cf_examples, random_state=2025)[modeling_features]
                print(f"Selected {num_cf_examples} rejected applicants for counterfactuals.")
                display(applicants_to_explain)

                # 6. Generate counterfactuals
                # Ask for 3 diverse counterfactuals per applicant, aiming for 'Approve'
                dice_exp = dice_explainer.generate_counterfactuals(
                     applicants_to_explain, 
                     total_CFs=3, 
                     desired_class="Approve",
                     # Optional: Specify feature ranges or permitted changes if needed
                     # features_to_vary=['dti', 'credit_score'] # Example
                )
                
                print("\nCounterfactual Explanations Generated:")
                # Display the explanations
                dice_exp.display_counterfactuals(show_only_changes=True) 
                dice_setup_ok = True # Flag that setup and generation succeeded

          else:
                print("No rejected applicants found in the test set to generate counterfactuals for.")
                applicants_to_explain = None # Ensure this is None if no applicants

     except Exception as e:
          print(f"Error generating counterfactual explanations: {e}")
          import traceback
          traceback.print_exc()
          applicants_to_explain = None # Ensure this is None on error

else:
     print("Skipping counterfactual explanations: dice-ml not installed or prerequisite data/model not available.")
     applicants_to_explain = None # Ensure this is None if skipped
