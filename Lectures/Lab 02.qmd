...
We will cover data preparation, assessing feature relationships, reject inference using fuzzy augmentation, training models with AutoGluon, diagnosing model behavior with ICE/PDP, applying monotonic constraints, evaluating performance using the KS statistic, selecting decision thresholds, generating adverse action reasons using counterfactual explanations, and interpreting predictions using SHAP.
...
def create_TTD_data(
    ...
    Args:
        ri_model: A trained scikit-learn compatible model used for reject inference.
                    ...
                    is skipped and all weights are set to 1.
...
features = [col for col in df_ttd_train.columns if col not in [label, weight_col, 'source']] # Exclude source too
...
ag_model_initial_wrapped = train_autogluon_model(
    ...
    modeling_features=features, # Use derived features list
...
# Get features from the initial model's wrapper
all_features = ag_model_initial_wrapped.feature_names_ # Use fitted feature names
...
# Ensure the order matches 'all_features' if using list for GBM
# Assuming order: ['loan_amnt', 'dti', 'credit_score', 'emp_length'] based on previous definition
monotone_constraints_dict = {'loan_amnt': 1, 'dti': 1, 'credit_score': -1, 'emp_length': -1}
# Create ordered list based on actual feature order from the wrapper
...
ag_model_constrained_wrapped = train_autogluon_model(
    ...
    label=label,
    weight_col=weight_col,
    modeling_features=all_features, # Use features from initial model
...
The optimal threshold is 767 (i.e., approve all applicants with the score >= 767, reject all applicants with the score < 767).
...
## 9.2. Evaluating the Threshold

We will evalulate the threshold using the test set. Up until now, we have used sample weights in the training, calibration, and test data sets. The weights were needed because the rejected population did not have a target outcome.

When we use the TTD model and the threshold in production, we are predicting the target outcome. The prediction does not need to know the true label.

Hence, to evaluate how the model will accept or reject applicants, we will use the test set without sample weights (setting all sample weights to 1) and without cloning the rejects.
