...
-   **Student Loans**: Unsecured, term loan. Federal student loans typically have fixed rates, while private student loans may have fixed or variable rates.
...
-   **Mathematical relationship**: Let $p$ be the probability of default. The score is calculated as:

$$\text{Score} = A - B \cdot \ln\left(\frac{p}{1-p}\right)$$

where:
    -   $\frac{p}{1-p}$ represents the **odds of default** (the ratio of default probability to non-default probability).
    -   $B$ is defined as $\text{PDO}/\ln(2)$. This factor converts log-odds into score points such that adding PDO points to the score exactly halves the odds of default (or equivalently, doubles the odds of being a "good" borrower).
    -   $A$ is a scaling constant (reference score) to position the score in the desired range.
-   **Key insight**: As the score increases by PDO points, the odds of default are halved. This means higher scores indicate lower default risk.
...
-   For PDO = 40:
    ...
    2.  If a 650 score corresponds to a 5% default probability ($p=0.05$):
        -   Compute the odds of default: $\frac{p}{1-p} = \frac{0.05}{0.95} \approx 0.0526$ and $\ln(0.0526) \approx -2.944$.
    ...
    4.  Verification: A score of 690 ($650 + 40$) should have odds of default equal to $0.0526 / 2 = 0.0263$, which corresponds to $p \approx 2.6\%$ default probability.
...
-   A higher PDO (like 40) means **more** points are needed to double the odds of good performance. A lower PDO (like 20) means **fewer** points are needed for the same effect, indicating a more sensitive or discriminating score where small score differences translate to larger changes in odds.
...
save_path = '../Artifacts/models/kgb_model'
...
# Use the best model from the trained ensemble
# Note: CatBoost may not be available if not trained, so we use the actual best model

# Get the actual best model name from the predictor
best_model_name = kgb_predictor.predictor.model_best
# kgb_predictor.predictor.set_model_best(best_model_name)  # Already set by training
...
save_path = '../Artifacts/models/augmented_all_bad_model'
...
save_path = '../Artifacts/models/augmented_bureau_proxy_model'
...
# Use the best model from the trained ensemble
best_model = bureau_proxy_predictor.predictor.model_best
# bureau_proxy_predictor.predictor.set_model_best(best_model)  # Already set by training
