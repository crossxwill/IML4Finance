...
ag_model_embedded_method = Pipeline([
    ('replace_values', FunctionTransformer(replace_vals)),
    ('prepare_features', FunctionTransformer(prepare_features)),
...
ag_model_embedded_method.fit( X = train_data_wide.drop(columns=[label]),
            y = train_data_wide[label]
            )
```
...
```{python}
df_leaders_embedded_method = ag_model_embedded_method.named_steps['autogluon'].predictor.leaderboard(test_data_wide)
...
ag_model_wrapper_method.fit(X = train_data_wide.drop(columns=[label]),
                        y = train_data_wide[label])
```
...
```{python}
df_leaders_wrapper_method = ag_model_wrapper_method.named_steps['autogluon'].predictor.leaderboard(test_data_wide)
