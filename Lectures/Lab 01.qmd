...
compare_profile_1.to_file("eda_report_ydata_1.html")
...
The treatment group has a much higher response rate than the control group.

The ROI of the treatment group is also much higher than the ROI of the control group.

```{python}
def calc_ROI(num_mailed, num_responded, avg_cost, avg_revenue):
    total_cost = num_mailed * avg_cost
...
Up until now, we used only a very small subset of the features.

```{python}
display(subset_cols)
```

We will now widen the data frames to include all features, and explore feature reduction methods.

```{python}
#| label: tbl-campaign-1-train-full
#| tbl-cap: "First 5 rows of df_campaign_1_train"
#| tbl-number: true


df_campaign_1_train.head()
```

```{python}
#| label: tbl-campaign-1-test-full
#| tbl-cap: "First 5 rows of df_campaign_1_test"
#| tbl-number: true


df_campaign_1_test.head()
```
...
ag_model_filter_method.fit( X = df_campaign_1_train.drop(columns=[label]),
            y = df_campaign_1_train[label]
            )
```
...
```{python}
df_leaders_filter_method = ag_model_filter_method.named_steps['autogluon'].predictor.leaderboard(df_campaign_1_test)
...
The chunk below creates a scikit-learn pipeline that uses `SelectFromModel` and the lasso to select the top features. The top features are then used to train an `autogluon` model.
...
ag_model_embedded_method.fit( X = df_campaign_1_train.drop(columns=[label]),
            y = df_campaign_1_train[label]
            )
```
...
```{python}
df_leaders_embedded_method = ag_model_embedded_method.named_steps['autogluon'].predictor.leaderboard(df_campaign_1_test)
...
The chunk below creates a scikit-learn pipeline that uses forward selection (`SequentialFeatureSelector`) and autogluon estimator to select the top features. The top features are then used to train an `autogluon` model.

```{python}
ag_model_wrapper_method = Pipeline([
    ('replace_values', FunctionTransformer(replace_vals)),
    ('feature_selection', SequentialFeatureSelector(
        AutoGluonSklearnWrapper(label=label,
                            predictor_args={'problem_type': 'binary',
                                            'eval_metric': 'roc_auc'},
                            fit_args={'holdout_frac': 0.2,
                                    'excluded_model_types': ['KNN'],
...
ag_model_wrapper_method.fit(X = df_campaign_1_train.drop(columns=[label]),
                          y = df_campaign_1_train[label])
```
...
```{python}
df_leaders_wrapper_method = ag_model_wrapper_method.named_steps['autogluon'].predictor.leaderboard(df_campaign_1_test)
