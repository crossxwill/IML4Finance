...
The section evalulates each strategy by using the tuning set and test set.
...
**Concept Recap:** Train the model on the original, imbalanced training data. We will use ROC AUC as the `eval_metric` for AutoGluon to select the best base models. Probability calibration and F1-score optimization via threshold tuning are key.
...
pre_cal_disp_s1.ax_.set_title("Pre-Calibration Plot")
```
...
post_cal_disp_s1 = CalibrationDisplay.from_estimator(
        estimator = cal_model_s1,
        X = X_test,
        y = y_test,
        n_bins = 20,
        name = 'Post-Calib Curve: Do Nothing Strategy',
        color = 'blue'
    )

post_cal_disp_s1.ax_.set_title("Post-Calibration Plot")
```
