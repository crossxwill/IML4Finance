...
df_train = (
        df_fraud
        .filter(pl.col("TX_DATETIME") <= pl.date(2018, 7, 31))
        )

# df_tuning: Transactions between 2018-08-01 (inclusive) and 2018-08-31 (inclusive)
df_tuning = (
        df_fraud
        .filter((pl.col("TX_DATETIME") >= pl.date(2018, 8, 1))
...
df_test = (
        df_fraud
        .filter(pl.col("TX_DATETIME") >= pl.date(2018, 9, 1))
        )
...
def fe_customer_spending(df: pl.DataFrame, window_sizes_in_days=[1, 7, 30]) -> pl.DataFrame:
    ...
    for window in window_sizes_in_days:
        output_df = (
            df_int
            .rolling(index_column="TX_DATETIME"
                , period=f"{window}d"
                , closed="both"
    ...
    for window in window_sizes_in_days:
        ...
        output_df = (
            ...
            .rolling(
                ...
                offset=f"-{full_offset}d"  # Apply the delay period
            )
            .agg(
                pl.len()
        ...
        # Join rolling features back to main df
        result_df = (
            result_df
            .join(output_df, on=["TERMINAL_ID", "TX_DATETIME"], how="left")
        )
