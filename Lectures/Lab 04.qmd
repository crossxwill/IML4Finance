...
This lab demonstrates how to use pipelines for feature engineering on credit card transaction data. The objective is to create new features that enhance the effectiveness of fraud detection models.
        ...
        result_df = (
            ...
            .with_columns(
                pl.col("TX_COUNT")
                    ...
                    .over("CUSTOMER_ID")
                .alias(f"CUSTOMER_ID_NB_TX_{window}DAY_WINDOW"),
                ...
                pl.col("TX_AMOUNT")
                    ...
                    .over("CUSTOMER_ID")
                .alias(f"CUSTOMER_ID_AVG_AMOUNT_{window}DAY_WINDOW")
            )
        )
...
Apply the data pipeline to `df_fraud` and then split the data into training and calibration sets. The test set will be created later.
...
Key modeling choices: \* We will use the `AutoGluonSklearnWrapper` for training. \* We will primarily use `average_precision` (PR-AUC) as the `eval_metric` for AutoGluon, as it's more robust for imbalanced fraud data. For the "Do Nothing" approach, we might start with `roc_auc` but will also closely monitor `average_precision`. \* Threshold tuning for F1-score will be performed on the `df_tuning_ft` set. \* Final F1-score will be reported on the `df_test_ft` set using the optimized threshold.
...
In machine learning, especially with complex workflows like fraud detection, **pipelines** are essential tools. They help you chain multiple data processing steps (e.g., feature scaling, encoding, feature engineering) and a final estimator (your model) into a single object.

Here's why pipelines are important for your learning and practice:

*   **Workflow Organization & Readability**:
    *   Pipelines clearly define the sequence of operations applied to your data.
    *   This makes your code cleaner, easier to understand, and less prone to errors. Imagine applying 5 feature engineering steps and then training a model – a pipeline neatly encapsulates this.

*   **Preventing Data Leakage (Crucial for Valid Evaluation)**:
    *   When performing operations like feature scaling or imputation, these should be learned *only* from the training data and then *applied* to the validation/test data.
    *   Pipelines, especially when used with cross-validation tools (like `GridSearchCV` or `cross_val_score`), ensure that data transformations are fitted correctly within each training fold, preventing information from the validation fold "leaking" into the training process. This leads to more reliable performance estimates.

*   **Reproducibility**:
    *   A pipeline bundles all your preprocessing and modeling steps. This makes it easier to reproduce your results consistently, as the exact sequence and configuration of steps are saved together.

*   **Simplified Hyperparameter Tuning**:
    *   You can tune hyperparameters of *both* the preprocessing steps and the model simultaneously using tools like `GridSearchCV` with a pipeline. For example, you could tune a parameter for a feature scaling method and a parameter for your classifier at the same time.

*   **Deployment-Ready**:
    *   A trained pipeline can be saved and loaded, making it easier to deploy your entire workflow (preprocessing + model) into a production environment.

**Types of Pipelines You'll Encounter:**

*   **`sklearn.pipeline.Pipeline`**: The standard scikit-learn pipeline, excellent for most preprocessing and modeling chains.
*   **`imblearn.pipeline.Pipeline`**: This version is specifically designed for workflows that include resampling steps (like SMOTE or undersampling) from the `imbalanced-learn` library. It correctly handles the `fit_resample` method of samplers, ensuring resampling is applied appropriately during cross-validation (only on the training part of each fold).

In this lab, while we perform feature engineering with Polars separately for efficiency, the concept of pipelining is vital when we move to modeling, especially if we were to integrate preprocessing steps directly with scikit-learn compatible models or when considering resampling strategies. The `AutoGluonSklearnWrapper` itself acts like a complex pipeline, managing many internal modeling steps.
...
**Process:** 1. Train the model on the original, imbalanced dataset. 2. During model training and selection (e.g., hyperparameter tuning in AutoGluon), use PR-AUC (`average_precision`) as the primary evaluation metric to guide the selection of the best model architecture and hyperparameters. 3. After training, check the probability calibration of the selected model. 4. Use a tuning set to choose an optimal classification threshold that maximizes a business-relevant metric like the F1-score.
