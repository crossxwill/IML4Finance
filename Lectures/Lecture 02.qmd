...
## History and Role of Credit Scores {.smaller}

1.  **Pre-1950s**: Credit decisions were subjective and often discriminatory
    -   Relied on personal relationships and judgment
    -   Loan officers assessed character, capacity, and capital ("3Cs")
    -   Resulted in inconsistent and biased lending practices
2.  **1958**: Fair Isaac Company (FICO) introduced the first credit scoring system
    -   Standardized and objectified lending decisions
    -   Initially resisted by traditional bankers
3.  **1970**: Fair Credit Reporting Act regulated the use of credit information

## Modern History of Credit Scores

1.  **1989**: Modern FICO scores adopted by major lenders
2.  **2006**: VantageScore launched by Equifax, Experian, and TransUnion
3.  **Today**: Credit scores influence:
    -   Lending decisions
    -   Insurance rates
...
VantageScore is unique as it was created jointly by the three major credit bureaus to compete with FICO.
...
-   **Definition**: PDO represents the number of credit score points required to double the odds of being a "good" borrower (or halve the odds of default).
-   **Example**:
    -   Reference Score: Score 680 with 10:1 odds of being "good."
    -   If PDO = 40:
...
-   **Mathematical relationship**: Let $p$ be the probability of default and define the odds as $\frac{p}{1-p}$; then: $$\text{Score} = A - B \cdot \ln\left(\frac{p}{1-p}\right)$$
    ...
    -   Note: This scaling ensures that an increase of PDO points in the score exactly doubles the odds of being a "good" borrower.
...
-   For PDO = 40:
    1.  Calculate $B$: $B = 40 / \ln(2) \approx 40 / 0.693 \approx 57.7$.
    2.  If a 650 score corresponds to a 5% default probability ($p=0.05$):
        ...
        -   Solve for $A$: $A \approx 650 - 169.7 \approx 480.3$.
    3.  Final formula: $$\text{Score} = 480.3 - 57.7 \cdot \ln\left(\frac{p}{1-p}\right)$$

## Industry Standards

-   $A$ is typically set to align scores with industry standards (e.g., score range of 300-850).
...
-   **Score Recalibration**: Models are periodically adjusted to maintain consistent risk rankings.
    -   Recalibration ensures that scores remain predictive as economic conditions and consumer behavior evolve.
...
-   **Definition**: The complete set of all consumers who apply for credit.
-   **Characteristics**:
...
Underwriting transforms the TTD population into accepted and rejected segments:
...
4.  **Decision**: Approve, conditionally approve, or reject.
    ...
    -   Conditional: Requires additional documentation or guarantor.
    -   Reject: Fails to meet minimum lending standards.
    -   Note: Some lenders may use "refer" status for borderline cases requiring manual review.
...
-   **Terminology**:
    ...
    -   "Known": Performance outcome is observed.
...
-   **Visual illustration**: In the risk curve below, KGB data only includes the green portion, missing the critical red portion:
...
```{python}
#| code-fold: false
#| label: risk-by-population-table
...
## Naive Augmentation: Results Comparison {.smaller}
...
# Impact of Augmentation on Model Performance {.smaller}
...
## Reducing Bias: Bureau Proxy Augmentation {.smaller}
...
# Simulate which rejected applicants the bureau would have data for
np.random.seed(42)
...
bureau_proxy_stats_data = {
    'Metric': [
        ...
        'Bureau proxy training set size',
        'Rejected applicants with bureau data',
        'Rejected applicants without bureau data (excluded)'
    ],
    'Value': [
        f"{len(X_ttd_train):,}",
...
## Bureau Proxy Augmentation: Results Comparison {.smaller}
...
```{python}
#| label: bureau-proxy-error-metrics
#| tbl-cap: "Bureau Proxy Error Metrics"
#| tbl-number: true

# Create a DataFrame for bureau proxy error metrics
bureau_proxy_error_data = {
    'Model': ['Bureau Proxy', 'Bureau Proxy'],
    'Score Range': ['Rejected (< 680)', 'Approved (>= 680)'],
    'MAE': [bureau_proxy_error_low_scores, bureau_proxy_error_high_scores]
}
bureau_proxy_error_df = pd.DataFrame(bureau_proxy_error_data)

# Format MAE as percentage
bureau_proxy_error_df['MAE'] = bureau_proxy_error_df['MAE'].map('{:.1%}'.format)

# Display the DataFrame
display(bureau_proxy_error_df)
```

# Key Takeaways {.smaller}

-   **Selection Bias**:
    -   Significantly impacts credit risk models when using only KGB data.
    -   Leads to underestimation of risk for lower credit scores.
    -   Creates a discontinuity in the risk curve at the approval threshold.
    -   Largest risk assessment gap occurs near the approval threshold.

-   **Reject Inference Techniques**:
    -   Substantially improve model accuracy.
    -   **Naive Augmentation**: Simple but overestimates risk.
